...
import { useIsDark, useChatThreadsState, useOnAgentManagerOpenFile, useOnAgentManagerOpenWalkthrough, useOnAgentManagerOpenContent, useWorkspaceFolders, useFileContent, useAccessor, useAllWorkspaces, useMultiWorkspaceStats, useSelectedWorkspace } from '../util/services.js';
import { SidebarChat } from '../sidebar-tsx/SidebarChat.js';
import { PastThreadsList } from '../sidebar-tsx/SidebarThreadSelector.js';
import { WorkspaceList, MiniWorkspaceList } from './WorkspaceList.js';
import { MultiWorkspaceThreadSelector } from './MultiWorkspaceThreadSelector.js';
import ErrorBoundary from '../sidebar-tsx/ErrorBoundary.js';
import { Folder, MessageSquare, Code, Settings, X, Maximize2, Search, ExternalLink, Activity, Shield, Cpu, Zap, ChevronLeft, ChevronRight, BarChart3, Layers, Clock, FileCode, Plus, Grid, Home, Terminal, Sparkles, Play, Pause, RefreshCw, Globe, Circle } from 'lucide-react';
...
/**
 * Multi-workspace view component
 */
const MultiView = ({ onNewThread, onOpenSettings }: {
	onNewThread: () => void;
	onOpenSettings: () => void;
}) => {
				...
				<div className="flex items-center justify-between mb-6">
					<div>
						<h2 className="text-xl font-bold text-void-fg-1 tracking-tight flex items-center gap-2">
							<Globe className="w-5 h-5 text-void-accent" />
							All Workspaces
						</h2>
						<p className="text-xs text-void-fg-4 mt-1 opacity-60">Monitor activity across all your VS Code windows</p>
					</div>
					<div className="flex items-center gap-2">
