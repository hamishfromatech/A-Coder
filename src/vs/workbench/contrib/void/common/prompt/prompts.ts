...
**Important:** Use webview_id returned by this tool in subsequent webview operations.`,
	...
	open_html: {
		name: 'open_html',
		description: `Opens HTML content directly in a new webview panel within VSCode. This is useful for displaying generated HTML, local HTML files, or any HTML content you want to preview.

**What you'll receive:** A webviewId (e.g., "wv1", "wv2") that can be used with other webview tools.

**When to use:**
- To preview HTML content generated by a-coder
- To display locally created HTML files
- To preview web app development in progress
- To render HTML from template engines

**Example:**
- open_html(html="<html><body><h1>Hello World</h1></body></html>") → Displays the HTML in a webview
- open_html(html=htmlContent, title="Preview") → Displays HTML with a custom title

**Important:** The HTML content is displayed with full access to the DOM, enabling get_page_text and webview_screenshot to work properly.`,
		params: {
			html: { description: 'Required. The HTML content to display.' },
			title: { description: 'Optional. A title for the webview tab. Defaults to "HTML Preview".' }
		}
	},
	fetch_url: {
		name: 'fetch_url',
		description: `Fetches the HTML content of a webpage. This bypasses CORS restrictions by using the main process.

**What you'll receive:** The full HTML content of the page, and optionally extracted text if extract_text is true.

**When to use:**
- To get the raw HTML of a page
- To analyze page structure
- When you don't need to interact with the page visually

**Example:**
- fetch_url(url="https://example.com", extract_text=true) → Returns HTML and extracted text
- fetch_url(url="https://example.com", extract_text=false) → Returns only HTML`,
		params: {
			url: { description: 'Required. The URL to fetch. Must be http:// or https://.' },
			extract_text: { description: 'Required. If true, extracts and returns the visible text from the page.' }
		}
	},
	open_devtools: {
		name: 'open_devtools',
...
**When to use:**
- When debugging JavaScript issues in a webview
...
- To click buttons and links
- To interact with forms

**Example:**
- click_element(webview_id="wv1", selector="button.submit") → Clicks the submit button
- click_element(webview_id="wv1", selector="a.next-page") → Clicks a link with class "next-page"
...
**When to use:**
- To read the content of a page without HTML markup
- To extract text from a specific section
- To analyze page content programmatically
...
- An AI-generated description of the screenshot
- Optional file path if saved

**When to use:**
- To visually inspect a webpage
- To debug visual layout issues
- To analyze web page design
- To understand the visual state of a page
...
**Important:** The webview must have been opened with open_url first. Requires a vision model to be configured.`,
	...
	search_web: {
		name: 'search_web',
		description: `Performs a web search and returns search results. Uses DuckDuckGo (no API key required).

**What you'll receive:** An array of search results with title, URL, and snippet for each result.

**When to use:**
- To find information online
- To search for documentation
- To research a topic
- To find examples or tutorials

**Example:**
- search_web(query="how to use React hooks", num_results=5) → Returns 5 search results
- search_web(query="Python async await") → Returns default number of search results`,
		params: {
			query: { description: 'Required. The search query.' },
			num_results: { description: 'Optional. Number of results to return (default: 10, max: 20).' }
		}
	},
	browse_resources: {
		name: 'browse_resources',
		description: `Lists the resources (CSS, JS, images) loaded by a webpage.

**What you'll receive:** An array of resources with type, URL, and size.

**When to use:**
- To analyze the page structure
- To find CSS/JS files
- To audit page assets
- To understand page dependencies

**Example:**
- browse_resources(url="https://example.com", resource_type="all") → Lists all resources
- browse_resources(url="https://example.com", resource_type="js") → Lists only JavaScript files`,
		params: {
			url: { description: 'Required. The URL of the page to analyze.' },
			resource_type: { description: 'Optional. Type of resources to list: "css", "js", "images", or "all" (default: "all").' }
		}
	},
	type_into_element: {
...
- To enter text into search boxes
- To input data into text areas
